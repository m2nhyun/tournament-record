...
2026-05-27 갱신 (snapshot 기준일 2026-05-22). 다음 작업자가 이어받아야 할 상태:
...
- native select/dropdown을 shadcn/Radix primitive로 전환 (완료, 2026-05-22)
  - 완료: shadcn/Radix `Popover`/`DropdownMenu`/`AlertDialog`로 모든 native control 교체
  - 검증: 2026-05-27 src 전체 grep 기준 `<select>` 0건, `<details>` 0건, `window.confirm` 0건 (club_record 영역 기준)
  - 교체 대상: 참가자 관리자, 경기 컨트롤, 일정 생성 시간 선택, 새 이벤트/이벤트 수정 시간 선택 (`ClubRecordTimeSelect`)

## Next Recommended Task Split

다음 작업자는 아래 중 하나만 골라 작은 단위로 끝낸다.

1. 새 `홈 / 이벤트 / 히스토리 / 클럽` IA를 운영진/멤버/게스트별로 Playwright/browser 실사용 검증
2. 월간 공개 카드 상세 화면 Playwright/browser 실사용 검증
3. 결과 입력 UX Playwright/browser 실사용 검증
4. legacy 라우트(`/matches/new`, `/history`, `/leaderboard`) 유지/폐기 정책 결정
