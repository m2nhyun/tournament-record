# Design System

## 목표

- 아이폰 소형 화면(최소 320px 폭)에서도 입력/조회가 명확해야 한다.
- 운영 화면은 시각적 강조보다 가독성과 조작 정확도를 우선한다.

## Design Tokens

`src/app/globals.css`의 CSS 변수 사용.

- `--brand`: 강조색
- `--brand-foreground`: 브랜드 배경 위 텍스트 색
- `--player-highlight`: 본인/조회 대상 선수 이름 강조색
- `--background`, `--foreground`: 기본 캔버스/텍스트
- `--surface-1`, `--surface-2`: 카드/배지 배경 계층
- `--border`, `--ring`: 경계/포커스 스타일

## Typography

- 기본 폰트: Pretendard > Noto Sans KR > 시스템 산세리프
- 제목: `font-semibold`, 좁은 화면에서 `text-3xl` 이하 유지
- 본문: 모바일 기본 `text-sm`, 줄간격 `leading-6`

## Spacing & Layout

- 최소 화면 폭: `320px`
- 모바일 기본 패딩: `px-4`
- 카드 내부 패딩: `p-4 ~ p-5`
- 데스크탑 확장 시 `max-w-*`로 읽기 폭 제한
- 페이지 기본 구조는 아래 계약을 따른다.
  - `<div><AppBar /><div className="px-4">...</div></div>`
  - `AppBar`는 상단 sticky, 콘텐츠 여백(`px-4`)은 본문 래퍼에서만 담당
- 클럽 화면처럼 하단 네비게이션이 있는 경우 `AppShell`은 `fixed inset-0` 캔버스로 유지하고, 전체 문서가 아니라 `AppShell > main`만 스크롤한다. 바텀 네비게이션은 화면 하단에 고정되고 스크롤 대상에 포함되지 않아야 한다.
- 루트 문서(`html`, `body`)는 뷰포트 높이로 고정하고 overflow를 잠근다. 페이지 전환과 스크롤은 앱 캔버스 내부에서만 처리한다.

## Components

- Button: `src/components/ui/button.tsx`
  - variant: `default`, `secondary`, `outline`
  - size: `default`, `sm`, `lg`
- 랜딩 하이라이트 카드: 섹션 카드 패턴 재사용 가능
- 클럽 계열 하단 네비게이션은 `홈`, `이벤트`, `히스토리`, `클럽` 4탭을 유지한다.
  - `홈` -> `/clubs/{clubId}`: club_record 대시보드 홈
  - `이벤트` -> `/clubs/{clubId}/club-record/events`: 안전한 이벤트 목록 진입
  - `이벤트` active 범위: `/clubs/{clubId}/club-record/events`, `/clubs/{clubId}/club-record/new`, 이벤트 상세/워크스페이스 라우트
  - `히스토리` -> `/clubs/{clubId}/club-record/history`: 회원 본인 기록 진입
  - `클럽` -> `/clubs/{clubId}/club`: 클럽 정보/초대/일정/멤버 관리
  - `홈` active 범위: `/clubs/{clubId}`, `/clubs/{clubId}/club-record`, `/clubs/{clubId}/club-record/monthly`, `/clubs/{clubId}/club-record/ranking`
  - 각 탭은 뒤로가기 없이 화면 목적에 맞는 우측 액션만 선택적으로 둔다.
  - 클럽 홈: `다른 클럽` 버튼
  - 히스토리: 확인 요청 알림 버튼
  - 이벤트/클럽: 기본적으로 우측 액션 없음
  - 뒤로가기는 상세/일회성 진입 화면에서만 사용한다.
- 클럽 회원 순위 화면은 `클럽 회원 랭킹` 명칭을 사용하고, 클럽 탭의 멤버 섹션 우측 `랭킹 관리` 액션을 주 진입으로 둔다.
- 랭킹 순서 조정은 현재 drag and drop 대신 명시 버튼을 사용해 모바일 스크롤 충돌을 피한다.
- 텍스트 강조 요청은 색상/굵기/배경처럼 요청된 범위 안에서만 처리한다. 이름이나 문장을 임의로 배지/칩 컴포넌트로 바꾸지 않는다.
- 같은 정보를 카드/리스트/요약 등 여러 보기에서 보여줄 때는 동일한 시각 문법을 유지한다. 한쪽만 배지, 한쪽만 텍스트처럼 갈라지지 않게 한다.
- 드롭다운, 메뉴, 선택, 팝오버, 다이얼로그, 탭, 토글은 shadcn/Radix primitive를 우선 사용한다. 새 UI에 native `<select>`나 브라우저 기본 드롭다운을 사용하지 않는다. 기존 화면에 남은 native control은 명시적 임시 유지 항목으로 문서화하고, 시간처럼 긴 옵션 목록은 `Popover` 기반 picker를 우선한다.
- 일정 생성 화면은 `핵심 결정 -> 세부 입력 -> 생성 전 요약` 순서를 유지한다.
- 일정 생성의 날짜/시간 입력은 다중 선택보다 `단일 일정 빠른 생성`을 우선하고, 캘린더 + 시작/종료 드롭다운 + 클릭형 시간칸이 같은 상태를 공유하도록 유지한다.
- 일정 생성은 모바일 기준 2단계 흐름을 기본으로 하고, 1단계는 날짜/시간/장소, 2단계는 비용/모집/메모로 분리한다.
- 장소/비용/정원처럼 참가 판단에 직접 영향을 주는 정보는 생성 전 요약 카드에서 다시 확인 가능해야 한다.

## Responsive Rules

1. 주요 CTA는 모바일에서 `w-full`로 터치 영역 확보
2. 멀티 컬럼은 모바일 1열 -> `sm` 이상에서 확장
3. 제목 줄바꿈은 좁은 폭 기준으로 먼저 설계
4. 조작성 우선: 텍스트 입력 최소 16px 체감 크기 유지

## Loading UX Rules

1. 공통 `LoadingSpinner`는 화면 중앙 정렬을 기본으로 사용한다.
2. 공통 로딩 상태는 스피너만 표시하고, 화면 설명 텍스트를 별도로 붙이지 않는다.
3. 데이터 의존 화면은 가능한 `EmptyState`를 사용해 "로딩 실패/데이터 없음/요건 미충족"을 분리해서 보여준다.

## 접근성 기준

- 포커스 링 유지 (`focus-visible:ring-*`)
- 텍스트/배경 대비 유지
- 버튼/카드 클릭 영역 충분히 확보
