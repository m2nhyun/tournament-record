...
1. **워크스페이스 시간대 그룹 헤더의 `열린 슬롯 N / 가능 인원 N` Badge 제거.** 어제 사용자가 같은 패턴 (`참가자 / 편성 가능 슬롯 / 미배정`) 을 무의미하다고 보고했고 시간대 그룹 헤더에도 같은 정보가 다시 노출되고 있어 함께 정리.
2. **슬롯 카드의 `슬롯 N · 예정` 에서 슬롯 번호 제거.** 슬롯 번호는 운영진도 의미를 못 알아본다는 사용자 의견 적용. 이제 `예정` 만 노출.
3. **`지난 이벤트에서는 새 경기를 만들 수 없습니다.` 메시지 슬롯별 반복 제거.** 같은 안내가 워크스페이스 상단 `StatusBox` 에 이미 한 번 떠 있어 슬롯마다 다시 노출되던 게 불필요한 반복이었다. `ClubRecordMatchControls` 에서 해당 분기 삭제.
4. **이벤트 탭 EmptyState 의 `새 이벤트 만들기` 버튼 제거.** 상단 안내 카드의 `새 이벤트` 버튼과 같은 액션이라 한 화면에 두 번 노출되던 중복. EmptyState 는 이제 텍스트만, CTA 는 상단 안내 카드 1곳.
5. **클럽 탭 멤버 행의 아이콘 버튼에 라벨 추가.** `Pencil` / `UserX` 아이콘만으로는 의미 불명이라는 평가. `내 정보 수정 → 설정`, `${닉네임} 클럽에서 제외 → 제외` 텍스트 + aria-label 추가.
6. **LoadingSpinner 의 title/message props 가 dead 였던 것 살림.** 컴포넌트가 props 를 받기만 하고 렌더에 안 쓰던 상태였다. 호출처는 이미 `LoadingSpinner title="로딩 중" message="..."` 같이 넘기고 있었지만 화면에는 스피너만 나왔다. 이제 텍스트가 같이 노출되어 빈 화면 + 가운데 동그라미만 보이던 마찰이 해소.
...
분석:
- **그룹 Badge**: `group_code` 는 자동 편성 알고리즘이 내부적으로 spread 점수를 계산할 때 쓰는 정보. 사용자가 직접 다루지 않고, 화면에서 보는 것 자체가 정보 누설/혼동 유발.
- **출석 수**: 회원이 이벤트 참가자로 등록된 횟수 카운터. 승패 정보 없이 출석 수만 단독 노출되면 의미가 모호.
- **확정 경기 수**: 자동 편성이 출전 횟수를 균등화하는 데 쓰는 내부 카운터. 사용자 입장에서 "확정 경기 수"라는 라벨로만 노출되면 무엇을 가리키는지 불명확.
- 세 항목 모두 자동 편성 알고리즘의 입력 변수일 뿐 운영진/회원이 행 단위로 시각적으로 확인할 가치가 낮음.

대응:
- `ClubRecordRankingView` 의 행을 `랭킹 번호 + 닉네임` 만 남겨 단순화.
- 카드 헤더 안내 텍스트의 "데일리 매치 편성과 그룹 배정에 사용하는 회원 순서입니다." → "자동 편성 시 우선순위로 사용하는 회원 순서입니다." 로 다듬어 그룹 개념 노출 제거.
- 카드 본문 하단의 "회원 순서 변경은 저장 즉시 데일리 매치 편성 기준에 반영됩니다." 안내는 중복이라 제거.
- 위/아래 화살표 버튼은 운영진(`canEdit`)만 보이는 핵심 순서 조정 액션이라 일단 유지. 다른 패턴(드래그앤드롭 등) 검토는 사용자 후속 의견 받고 진행.
