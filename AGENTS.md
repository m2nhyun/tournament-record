...
- 현재 운영 정책상 실제 반영은 CLI(`db:push`)를 우선하고, Supabase `SQL Editor` 수동 실행은 Dashboard-only 변경의 예외 경로로 둔다.
...
- DB 작업 판단 규칙:
  ...
  - 대시보드 수동 반영 뒤 로컬 schema만 늦으면 `db:schema:sync`
...
- `npm run db:push:dry` 결과가 예상과 일치하는지 확인
- remote schema는 이미 있는데 migration history만 비어 있으면 `supabase migration repair --status applied` 우선 검토
- `npm run db:push` 또는 적절한 repair 실행 여부 확인
